...
const executeRoute = (req,res,params,route) => {
    const routeMiddlewareExecution = (req,res,middlewareList,params,cb) => {
        let j = 0;
        let mc = 0
    ...
    if(globalMiddleware.length>0){
                ...
                if(go){
                    if(j===globalMiddlewareList.length-1){
                        if(routeMiddlewareList.length>0){
                            routeMiddlewareExecution(req,res,routeMiddlewareList,params,cb)
                        }else{
                            callCallback(cb,req,res,params)
                        }
                    }
        ...
        if(route.middleware===undefined){
            ...
            if(middlewareList.length>0){
                routeMiddlewareExecution(req,res,middlewareList,params,cb)
            }else{
                callCallback(cb,req,res,params)
            }
        }
